...
class Config:
    ...
    MAX_AGE_MIN = 20           # Не публікувати новини старше 20 хвилин (Топові/Свіжі) - ЗМЕНШЕНО
    
    # 🛡️ ПАРАМЕТРИ НАДІЙНОСТІ ТА ПРОДУКТИВНОСТІ (Покращено)
    FETCH_LIMIT = 50           # Макс. кількість записів для обробки з одного RSS-фіда - ЗБІЛЬШЕНО
    NUM_SOURCES_TO_FETCH = 30  # Кількість випадкових джерел, які парсяться за цикл - ЗБІЛЬШЕНО
    HTTP_TIMEOUT = 10          # Таймаут для HTTP-запитів
    MAX_CONCURRENCY = 30       # Макс. одночасних з'єднань для парсингу (Збільшено для швидкості)
    ...
    DB_POOL_MAX = 3            # Максимальний розмір пулу (ЗМЕНШЕНО для економії Compute Time Neon)
